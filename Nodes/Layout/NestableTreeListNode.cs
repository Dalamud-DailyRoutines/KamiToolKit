...
public class NestableTreeListNode<T, TU> : ResNode where TU : TreeListItemNode<T>, ITreeListItemNode, new()
{
    ...
    /// </summary>
    /// <remarks>
    ///     When changing the list data, you will probably need to invoke this manually if <see cref="AutoResetScroll" /> is
    ///     disabled.
    /// </remarks>
    ...
    private IEnumerable<NestableTreeListRow<T>> EnumerateVisibleRows()
    {
        ...
        {
            foreach (var row in EnumerateSectionRows(section, default))
                yield return row;
        }
    }

    private IEnumerable<NestableTreeListRow<T>> EnumerateSectionRows
    (
        TreeListSection<T> section,
        ReadOnlySeString   parentPath
    )
    {
    ...
    /// </summary>
    private unsafe void OnScrollUpdate
    (
        int newPosition
    )
    {
    ...
    /// </summary>
    private unsafe void OnMouseWheel
    (
    ...
    (
        NestableTreeListRow<T> row
    )
        => row.Kind is NestableTreeListRowKind.Group ?
               28.0f :
               itemHeight;
}
