...
public unsafe class ColorRingWithSquareNode : SimpleComponentNode
{
    ...
    {
        get => ColorGradientSquare.MultiplyColorHsva;
        set => ColorGradientSquare.ColorDotPosition = new Vector2
                   (ColorGradientSquare.Width * value.S, ColorGradientSquare.Height - (ColorGradientSquare.Height * value.V));
    }
    ...
    private bool IsRingClicked
    (
        AtkEventData* data
    )
    {
        var clickPosition = data->MousePosition;
        var scale = ParentAddon is not null ?
                        ParentAddon->Scale :
                        1.0f;
    ...
    private float GetRingClickAngle
    (
        AtkEventData* data
    )
    {
    ...
    private void SquareEventHandler
    (
        AtkEventListener* thisPtr,
        AtkEventType      eventType,
        int               eventParam,
    ...
    private static Vector2 RotatePoint
    (
