...
///     Animations can only be run and defined by parent nodes, it's not possible to self contain everything you need to
///     animation.
/// </remarks>
public class FrameSetBuilder
(
    TimelineBuilder parent,
    int             startFrameId,
    int             endFrameId
)
{
    ...
    (
        params TimelineKeyFrame[] keyFrame
    )
    {
    ...
    /// </summary>
    public FrameSetBuilder AddEmptyFrame
    (
        int frameId
    )
    {
    ...
    (
        ...
        byte?                     alpha            = null,
        Vector3?                  addColor         = null,
        Vector3?                  multiplyColor    = null,
        float?                    rotation         = null,
        Vector2?                  scale            = null,
        Vector3?                  textColor        = null,
        Vector3?                  textOutlineColor = null,
        uint?                     partId           = null,
        AtkTimelineInterpolation? interpolation    = null,
    ...
    )
    {
        ...
        if (addColor is not null || multiplyColor is not null)
        {
            ...
            (
                new TimelineAnimationKeyFrame
                {
                    FrameIndex = frameId, AddColor = addColor ?? new Vector3(0.0f, 0.0f, 0.0f), MultiplyColor = multiplyColor ?? new Vector3(100.0f, 100.0f, 100.0f),
    ...
    /// </remarks>
    public FrameSetBuilder AddLabel
    (
        int                     frameId,
        int                     labelId,
        AtkTimelineJumpBehavior jumpBehavior,
        int                     labelTarget
    )
    {
        labelKeyFrames.Add
        (
            new TimelineLabelSetKeyFrame
            {
                FrameIndex   = frameId,
    ...
    /// </summary>
    public FrameSetBuilder AddLabelPair
    (
        int frameStart,
        int frameStop,
        int labelId
    ...
    /// </summary>
    public KeyFrameBuilder BeginFrameBuilder
    (
        int frame
    )
        => new(this, frame);
